		...
		<view class="danmu-li" v-for="(item,index) in listData" :class="item.type" :style="[item.style]" :key="index">
			<view class="danmu-inner">
				<view class="user-box">
					<view class="user-img">
						<view class="img-box">
							<image :src="item.user && item.user.avatar"></image>
						</view>
					</view>
					<view class="user-text cl1">
					  {{item.user && item.user.nickName | pureNick}}
					</view>
		...
		methods: {
			...
			add(obj,index) {
				...
				let data = {
					...
					time:this.minTime*index,
					// time: Math.ceil(Math.floor(Math.random() * (this.maxTime - this.minTime + 1) + this.minTime*index)),
					type: this.type
				}
