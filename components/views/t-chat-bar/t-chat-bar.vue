	...
	<view @touchmove.stop.prevent>
		<view class="tui-reply-tabbar" :style="{ paddingBottom: `${keyboardHeight}px` }">
			<view class="tui-chat-tabbar">
				<view class="tui-icon tui-icon-im_more tui-icon-mr" @click="showKeyBoard(3)"></view>
				<view class="tui-icon tui-icon-im_face tui-icon-mr" @click="showKeyBoard(2)"></view>
				<!-- v-show="showIndex != 2" -->
				<!-- <view class="tui-icon tui-icon-im_keyboard  tui-icon-mr" @click="showKeyBoard(1)" v-show="showIndex == 2"></view> -->
				...
				<view class="tui-send-box">
					<view class="tui-icon tui-icon-im_voice" v-if="!isVoice && !content" @click="switchVoice"></view>
					<view class="tui-icon tui-icon-im_keyboard" v-if="isVoice && !content" @click="switchInput"></view>
					<view class="tui-btn-send" v-if="content" :style="{ color: color }" hover-class="tui-opcity" :hover-stay-time="150" @clicks="hideKeyboard">{{ sendText }}</view>
				</view>
			</view>
				...
				<view class="tui-face-box" v-if="showIndex == 2">
					<scroll-view scroll-y :style="{ height: replyContainerH + 'px' }">
						<view class="tui-face-scroll">
							<view class="tui-face-img_box" v-for="(item, index) in faceList" :key="index" @click="inputFace(index)">
...
export default {
	...
	data() {
		return {
			...
			keyboardHeight: 0,
			replyContainerH: uni.rpx2px(500),
			faceList: [],
			content: '',
			isVoice: false
		};
