	...
	<view @touchmove.stop.prevent="stop">
		<view class="tui-bottom-navigation" :class="{ 'tui-navigation-fixed': isFixed, 'tui-remove-splitLine': unlined }">
			<view
				...
				:key="index"
			>
				<view class="tui-item-inner" @click="menuClick(index, item.parameter, item.type)">
					<image
				...
				>
					<view
						...
						:key="subIndex"
						@click="subMenuClick(index, item.type, subIndex, subItem.parameter)"
					>
		...
		<view class="tui-navigation-mask" :class="{ 'tui-navigation-mask_show': showMenuIndex != -1 }" @click="handleClose"></view>
	</view>
</template>
