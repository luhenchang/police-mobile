<template>
	<view class="container">
			...
			<u-tabs :list="tabList" :current="tabCurrent" font-size="28"  class="b-b" :is-scroll="false" :active-color="tempColor" @change="tabChange"></u-tabs>
			<swiper :current="tabCurrent" class="swiper-box" duration="300" @change="changeTab">
				<swiper-item class="tab-content" v-for="(item,index) in tabList" :key="index">
					...
					<view class="search-wrap" v-if="item.type==3">
						<u-search placeholder="搜索涉案财物/物品名称/物品编号/案件名称" bg-color="#ffffff"   @click="search" :show-action="false" disabled></u-search>
					</view>
					...
					<xui-case-police-list v-if="item.type==0" :list="item.list" :showType="2" @reload="loadData()" />
...
<script>
	...
	export default {
		...
		created() {
			this.tabCurrent=this.policeTab;
			this.loadData();
		},
		onPullDownRefresh() {
			this.loadData(false);
